...
def main() -> None:
    ...
    if args.once:
        ...
        # Route through make_client (not a bare mqtt.Client) so the once-shot
        # gets a Last-Will + connect-failure logging, matching the daemon path
        # (BasePublisher.run) and the snmp/ipmi --once paths.
        client = make_client(
            config, collector.client_id, will_topic=collector.avail_topic
        )
